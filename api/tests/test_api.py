...
def test_load_datastore():
    response = client.get("/dijkstra/load-datastore", headers=AUTH)
    assert response.status_code == 200
...
def test_shortest_path_missing_params():
    response = client.get("/dijkstra/shortest-path", params={"from_node": "bugey"}, headers=AUTH)
    assert response.status_code == 422


def test_rapport():
    response = client.get("/dijkstra/rapport", headers=AUTH)
    assert response.status_code == 200
...
def test_liste_centrales():
    response = client.get("/dijkstra/centrales", headers=AUTH)
    assert response.status_code == 200
...
def test_get_centrale_ok():
    response = client.get("/dijkstra/centrales/bugey", headers=AUTH)
    assert response.status_code == 200
...
def test_get_centrale_unknown():
    response = client.get("/dijkstra/centrales/atlantide", headers=AUTH)
    assert response.status_code == 404


def test_liste_regions():
    response = client.get("/dijkstra/regions", headers=AUTH)
    assert response.status_code == 200
...
def test_get_region_ok():
    response = client.get("/dijkstra/regions/ile_de_france", headers=AUTH)
    assert response.status_code == 200
    assert response.json()["id"] == "ile_de_france"


def test_get_region_unknown():
    response = client.get("/dijkstra/regions/atlantide", headers=AUTH)
    assert response.status_code == 404


def test_liste_liaisons():
    response = client.get("/dijkstra/liaisons", headers=AUTH)
    assert response.status_code == 200
...
def test_anomalies():
    response = client.get("/dijkstra/anomalies", headers=AUTH)
    assert response.status_code == 200
...
def test_calcule():
    ...
    body = response.json()
    assert body["region"] == "ile_de_france"
    assert "repartition" in body
    assert "puissance_manquante_mw" in body


def test_calcule_unknown_region():
    response = client.get(
        "/dijkstra/calcule",
