...
# appelé en HTTP quand ms_metier en a besoin.
...
class DataStore:
    ...
    def verify(self):
        ...
        centrales_reliees = set()
        for liaison in self.liaisons:
            centrales_reliees.add(liaison.from_id)
            centrales_reliees.add(liaison.to_id)
        isolees = [
            centrale_id for centrale_id in self.centrales
            if centrale_id not in centrales_reliees
        ]
        if isolees:
